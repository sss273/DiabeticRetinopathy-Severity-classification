...
# paths
train_path = '/media/sandeep/New Volume/Angel Billy/DiabeticRetinopathy train'
valid_path = '/media/sandeep/New Volume/Angel Billy/Final dataset/val'
model_path = '/home/sandeep/PycharmProjects/FireDetection/diabetic_retinopathy/MODEL/saved_models/'
callbacks_path = '/home/sandeep/PycharmProjects/FireDetection/diabetic_retinopathy/MODEL/saved_models/logs/'
    ...
    for lr_rate in learning_rate:
        for bs in Batch_size:
            for lyr in layers:
                ...
                adam_amsgrad = Adam(lr=lr_rate, amsgrad=False)
                model.compile(optimizer=adam_amsgrad, loss='categorical_crossentropy', metrics=['accuracy'])
                model.summary()

                # Model path
                savedname = "MN214_DR_drop_" + str(dropout / 100) + "_lr_" + str(lr_rate) + "_BS_" + str(
                    bs) + "_FTL_" + str(lyr)
                ...
                cp_callback = ModelCheckpoint(modelckpt, monitor='val_loss', verbose=0, save_best_only=True,
